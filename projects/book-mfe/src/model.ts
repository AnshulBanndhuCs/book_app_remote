...
export class BookClient {
  ...
  uploadImage(
    id: number,
    bookImage: FileParameter | null | undefined
  ): Observable<FileResponse | null> {
    let url_ = this.baseUrl + '/api/Book/UploadImage/{id}';
    if (id === undefined || id === null)
      throw new Error("The parameter 'id' must be defined.");
    url_ = url_.replace('{id}', encodeURIComponent('' + id));
    url_ = url_.replace(/[?&]$/, '');
    ...
    if (bookImage !== null && bookImage !== undefined)
      content_.append(
        'bookImage',
        bookImage.data,
        bookImage.fileName ? bookImage.fileName : 'bookImage'
      );
